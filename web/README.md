...
| 文件传输 | 文件上传/删除、发起传输、任务两段式进度（下载段 downloadPercent + BLE 传输段 percent，2s 轮询）与取消、日志上传触发 |
| 测试编排 | 场景管理（查看/编辑/新建/另存为/删除）、JSON 编辑器（格式化/本地语法校验/422 错误展示）、场景运行（path/内联 JSON/保存并运行）、运行历史、步骤级报告（P50/P95）、停止 |
